...
def disambiguate_entity_executor(llm: LLMBase, entities: List[Entity], expert: str='', strategy:Strategy = Strategy.accuracy) -> List[Entity]:
    ...
    entity_type_dict = merge_summary_entity(llm, entity_type_dict, expert, strategy)
...
if __name__ == "__main__":
    ...
    output_filepath = "./examples/graphfiles/Gullivers-travels-A-Voyage-to-Lilliput.json"
    ...
    # multi thread
    parallel_chunks_args_list = [(aoai_llm, chunks[i], chunk_ids[i], expert, strategy) for i in range(0, 50)]
    paralled_chunks_graph_result_list = parallel_for(generate_chunk_graph_executor, parallel_chunks_args_list, num_threads=10)
    relations, entities = [], []
    ...
    graph_save(entities, relations, output_filepath)
