...
def generate_expert(llm: LLMBase, chunk: Union[str, List[str]], chunk_select_num: int=3) -> str:
    """Generate an LLM expert description to use for GraphRAG prompts.

    Parameters
    ----------
    - llm (CompletionLLM): The LLM to use for generation
    - domain (str): The domain to generate a expert description.
    - task (str): The task to generate a expert description. Default is DEFAULT_TASK
    """
